...
public class Customer_servlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		}else if(command.equals("idandphone")) { // 비밀번호 찾기
			
		    System.out.println("idandphone 체크입니다.");
		    
		    String id = req.getParameter("id");
		    
		    String phone = req.getParameter("phone");

		    boolean check = dao.getidandphonecheck(id, phone);

		    if(check == true){
		    	System.out.println("아이디와 폰번호가 일치함");
		    }else{
		    	System.out.println("아이디와 폰번호가 일치하지 않음");
		    }
