...
public class mainClass {
	...
	public static void main(String[] args) {
		// 알고리즘 4대 요소
		// 정렬(sort), 트리(tree), 셔플(shuffle), 탐색(index)
		/*
		
		int number[] = new int[52];
		boolean swit[] = new boolean[52];
		
		// 본인이 선언 후 초기화를 할지 선언가 동시에 초기화를 할지는 보인 선택에 달려있다.
		...
		*/
		...
		int cardNumber; 	// 1~13 카드 번호를 저장할 변수
		int cardPicture;	// 0~3 카드 종류를 저장할 변수
		...
		System.out.print("r number : "+number[i] + " ");
		
		
		
		cardNumber = number[i] % 13 + 1; // 1 ~ 13
		
		if(cardNumber == 1) {
			System.out.print("실숫자 :" + "A" + " ");
		}
		else if (cardNumber == 11) {
			System.out.print("실숫자 :" + "J" + " ");
		}
		else if (cardNumber == 12) {
			System.out.print("실숫자 :" + "Q" + " ");
		}
		else if (cardNumber == 13) {
			System.out.print("실숫자 :" + "K" + " ");
		}
		else {
			System.out.print("실숫자 :" + cardNumber + " ");
		}
		
		
		cardPicture = number[i] / 13;
		
		switch(cardPicture) {
			case 0:
				System.out.print("카드 문양 : ♠ ");
				break;
			case 1:
				System.out.print("카드 문양 : ◆ ");
				break;
		...
		// 0~51 숫자 : number[] + 1 = 1 ~ 52
		// 1 -> A 2~10 2 ~ 10 11-> j...q...k
