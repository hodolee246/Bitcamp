...
public class jdbcTest {
	...
	public Connection getConnection() {
		...
		try {
			conn = DriverManager.getConnection("jdbc:oracle:thin:@192.168.0.47:1521:xe", "hr", "hr"); // 커넥션 주소 및 연결할
																										// 아이디 패스워드
	...
	public int Insert(String id, String name, int age) {
		...
		System.out.println("sql :" + sql); // sql 문장은 , ' " 하나로 오류가 발생하기 때문에 왠만하면 확인한다.
		try {
			...
			// Insert문이 돌아가면 돌아간 횟수만큼 count값이 증가한다.
		} catch (SQLException e) {
			e.printStackTrace();
		} finally { // 무조건 실행시키게 해서 전부다 사용후 커넥션 스테이터스 를 닫아줘야한다.
			try { // 이런식으로 if 문 밖으로 try catch 문을 밖으로 뺄경우 하나의 try / catch 문으로 여러개의 if문을 실행할수있다.
				if (stmt != null) {
					stmt.close();
				}
				if (conn != null) {
